...
object MasterApp extends App {
  ...
  val config = ConfigFactory.parseString(
    """
      |akka.remote.artery.canonical.port = 3551
    """.stripMargin)
    .withFallback(ConfigFactory.load("udemyRemoteActorsExercise.conf"))
...
object WorkersApp extends App {
  val config = ConfigFactory.parseString(
    """
      |akka.remote.artery.canonical.port = 3552
    """.stripMargin)
    .withFallback(ConfigFactory.load("udemyRemoteActorsExercise.conf"))
