package test.akka_cluster_example
...
object WordCountDomain {
  final case class Initialize(nWorkers: Int) extends OnoSerializable
  final case class WordCountTask(text: String) extends OnoSerializable
  final case class WordCountResult(count: Int) extends OnoSerializable
  final case class EndWordCount() extends OnoSerializable
}
  ...
  def initializing(workers: List[ActorRef], remainingWorkers: Int): Receive = {
    case ActorIdentity(42, Some(actorRef)) =>
      if (remainingWorkers == 1)
        ...
        context.become( initializing(actorRef :: workers, remainingWorkers - 1 ) )
  }
...
object MasterApp extends App {
  ...
  val config = ConfigFactory.parseString(
    """
      ...
      |""".stripMargin)
    .withFallback(ConfigFactory.load("akka_cluster_example.conf"))
  ...
  scala.io.Source.fromFile("/tmp/lipsum.txt").getLines().foreach{ line =>
    master ! line
  }
}
...
object WorkerApp extends App {
  ...
  //(1 to 5).map( i=>  system.actorOf(Props[WordCountMaster], s"WordCountWorker$i") )
